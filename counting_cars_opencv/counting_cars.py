...
backsub = cv2.createBackgroundSubtractorMOG2()  #arka planı temizliyoruz mog2 yontemiyle
capture = cv2.VideoCapture("video.avi")
i = 0
minArea = 2600
while True:
    ret, frame = capture.read()  #kaydı aldık

    fgmask = backsub.apply(frame, None, 0.02)       #on goruntu maskeleme işlemi
    erode = cv2.erode(fgmask, None, iterations=4)   #erode metoduyla goruntudeki gurultuleri(parazitleri) temizliyoruz
    ...
    if moments['m00'] >= minArea:
        ...
        cv2.putText(frame, 'Arac: %r' % i, (200, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
